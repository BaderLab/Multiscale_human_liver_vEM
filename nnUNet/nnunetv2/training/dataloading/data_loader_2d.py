...
class nnUNetDataLoader2D(nnUNetDataLoaderBase):
    def generate_train_batch(self):
        # randomly choose the image within the imagedataset
        # the selected number of image is the same as the batch size. 
        selected_keys = self.get_indices()
        # preallocate memory for data and seg
        data_all = np.zeros(self.data_shape, dtype=np.float32)
        seg_all = np.zeros(self.seg_shape, dtype=np.int16)
        case_properties = []
        #print("selectttt:", selected_keys) # selected_keys have the same size of the batch_size:['mouse_000' 'mouse_000']
        for j, current_key in enumerate(selected_keys):
            ...
            #print("dddddddddsssppp", data.shape, seg.shape, properties) # (1,1,6263,6263)
            ...
            shape = data.shape[1:]
            dim = len(shape)
            # get_bbox randomly pick up the region for cropping and patchifying the images. 
            ...
            #print("thissliceeee:", this_slice)  # (slice(0, 1, None), slice(3347, 4852, None), slice(4352, 5857, None)), crop region of each dimension, each time is different region
        ...
        if self.transforms is not None:
            ...
            #print('dataallshapeeeeeeee:', data_all.shape)
