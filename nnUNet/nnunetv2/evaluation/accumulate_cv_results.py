...
def accumulate_cv_results(trained_model_folder,
                          ...
                          overwrite: bool = True):
    """
    for each one model, integrate all the prediction folder-wide 
    ...
    # integrate all the folds together
    for f in folds:
    ...
    if did_we_copy_something or not isfile(join(merged_output_folder, 'summary.json')):
        print("we want to create the summary.json!")
        label_manager = plans_manager.get_label_manager(dataset_json)
        gt_folder = join(nnUNet_raw, plans_manager.dataset_name, 'labelsTr')
        if not isdir(gt_folder):
            print("we do not have ground truth labelsTr folder! changed to preprocessed one!")
