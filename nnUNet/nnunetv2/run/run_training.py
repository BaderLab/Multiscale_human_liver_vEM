...
from nnunetv2.run.freeze_weight import freeze_weight
...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
                          pretrained_weights_file: str = None, freeze: bool=False):
    ...
    if freeze == True:
        freeze_weight(nnunet_trainer.network)
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        # mp.spawn will handle the process spawning, 
        # so only need one task per node in the SLURM configuration
        mp.spawn(run_ddp,
                 args=(
                     dataset_name_or_id,
                     configuration,
        ...
        maybe_load_checkpoint(nnunet_trainer, continue_training, only_run_validation, pretrained_weights, freeze)
...
def run_training_entry():
    ...
    parser.add_argument('--freeze', action='store_true', required=False,
                        help='[OPTIONAL] freeze the weight of encoder')
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights, args.freeze,
